(function () {
  ...
  style.textContent = `
    ...
    .case-panel{width:min(1080px,96vw);max-height:min(840px,92dvh);overflow:auto;color:#e9ebf3;background:linear-gradient(145deg,#171b25,#0e1118);border:1px solid #ffffff20;border-radius:18px;box-shadow:0 26px 90px #000a;padding:24px;scrollbar-gutter:stable}
    ...
    .recovery-grid{display:grid;grid-template-columns:minmax(250px,.72fr) minmax(0,1.8fr);gap:18px;min-height:0}.recovery-sidebar{display:grid;align-content:start;gap:12px}.recovery-section{border:1px solid #ffffff18;border-radius:13px;background:#ffffff05;padding:14px}.recovery-section h2{font-size:15px}.recovery-records{display:grid;gap:7px;max-height:340px;overflow:auto;padding-right:4px}.recovery-record{border-left:2px solid #7890b6;padding:5px 0 5px 10px}.recovery-record strong{display:block;font-size:13px}.recovery-record small{color:#8fa3c4}.recovery-empty{font-size:13px;color:#8e98a9!important}
  ...
  function snapshotsMarkup(current) {
    const snapshots = Object.values(current.snapshots || {});
    if (!snapshots.length) return '<p class="recovery-empty">保存版は、初版を確認したページから順に追加されます。</p>';
    return snapshots.map((item) => `<article class="snapshot-row"><strong>${escapeHtml(item.url)}</strong><p>保存版 ${escapeHtml(item.version)} · ${escapeHtml(item.capturedAt)}</p></article>`).join("");
  }
  ...
  function claimMarkup(id, claim, current) {
    ...
    const eligible = dependencyReady && observed.length >= claim.min && distinctMedia(observed, current) >= 2;
    ...
    if (!eligible) {
      return `<article class="claim-card is-current" data-claim="${id}"><header><small>整理中</small><h2>${escapeHtml(claim.stage)}</h2></header><p>記録の関連付けに必要な資料が不足しています。</p></article>`;
    }
    ...
    return `<article class="claim-card is-current" data-claim="${id}"><header><small>照合可能</small><h2>${escapeHtml(claim.stage)}</h2></header><p class="claim-question">${escapeHtml(claim.question)}</p><div class="interpretation-options">${options}</div><h3>根拠に使う資料</h3><div class="evidence-picks">${picks}</div><button class="claim-submit" type="button" data-accept-claim="${id}">選んだ関係を保存</button><p class="claim-status" role="status"></p></article>`;
  }
    ...
    layer.innerHTML = `
      <article class="case-panel">
        ...
        <div class="recovery-tabs" role="tablist">
          ...
          <button type="button" role="tab" data-recovery-tab="hints" aria-selected="false">任意ヒント</button>
        </div>
        <div class="recovery-grid">
          <aside class="recovery-sidebar">
            <section class="recovery-section"><h2>確認した資料</h2><div class="recovery-records">${recordsMarkup(current)}</div></section>
            <section class="recovery-section"><h2>関係参照</h2><div class="relationship-map">${relationshipNodes(current).map((node) => `<span>${escapeHtml(node)}</span>`).join("")}</div></section>
          </aside>
          <main>
            <section data-recovery-panel="claims" class="claim-workspace">${claimEntries.map(([id, claim]) => claimMarkup(id, claim, current)).join("")}</section>
            <section data-recovery-panel="copies" hidden><h2>ページ保存版</h2><p>開いたページはその時点の表示を保持します。再読込や新規検索は現行版を解決します。</p>${snapshotsMarkup(current)}</section>
            <section data-recovery-panel="hints" hidden><h2>任意ヒント</h2><p>答えではなく、いま比較できる観察対象から順に開きます。</p>${hintMarkup(current)}</section>
            <section data-ending-host></section>
          </main>
        </div>
      </article>`;
    document.body.appendChild(layer);
  ...
  function renderEndings(host, current) {
    if (!current.claims.I8) { host.innerHTML = ""; return; }
    host.innerHTML = `
      <hr>
  ...
  window.addEventListener("case-state-change", (event) => {
    ...
    if (button) button.dataset.alert = String(completeEvidenceIds(event.detail || state()).length > 0);
