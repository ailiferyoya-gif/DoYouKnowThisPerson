...
const S=A.appState({tabs:[initial],active:"t1"});let tabs=Array.isArray(S.tabs)&&S.tabs.length?S.tabs:[initial],active=S.active||tabs[0].id;
...
function drawTabs(){root.querySelector(".tab-strip").innerHTML=tabs.map(t=>'<button class="tab '+(t.id===active?"active":"")+'" data-tab="'+t.id+'">'+A.esc(t.title||"新しいタブ")+'</button>').join("");root.querySelectorAll("[data-tab]").forEach(b=>b.onclick=()=>{active=b.dataset.tab;save();const entry=current();entry?renderEntry(entry):push("vdm://home");});}
...
function renderEntry(entry){address.value=entry.url;if(entry.kind==="home")home();else if(entry.kind==="error")page.innerHTML='<div class="browser-error"><h2>ページを表示できません</h2><p>このアドレスは端末内に保存されていません。</p><button class="ghost" data-home>ホームへ</button></div>',page.querySelector("[data-home]").onclick=()=>push("vdm://home");else if(entry.kind==="deleted")page.innerHTML='<div class="browser-error"><h2>404</h2><p>指定された記録は削除されています。</p></div>';else{const renderer=VDM_SITE_RENDERERS[entry.preset];page.innerHTML=renderer?renderer(entry.site,entry.snapshot,{route:entry.url,query:new URLSearchParams(entry.query)}):'<div class="browser-error"><h2>'+A.esc(entry.title)+'</h2></div>';VDMEvidence.observe(entry.snapshot,"open-page");bindPage();if(entry.fragment){const target=page.querySelector("#"+CSS.escape(entry.fragment));target?.scrollIntoView();}}tab().title=entry.title;drawTabs();save();}
root.querySelector("[data-address-form]").onsubmit=e=>{e.preventDefault();push(address.value);};root.querySelector("[data-back]").onclick=()=>{const t=tab();if(t.cursor>0)t.cursor-=1;const entry=current();save();if(entry){renderEntry(entry);emitRoute("back",entry);}};root.querySelector("[data-forward]").onclick=()=>{const t=tab();if(t.cursor+1<t.history.length)t.cursor+=1;const entry=current();save();if(entry){renderEntry(entry);emitRoute("forward",entry);}};root.querySelector("[data-reload]").onclick=()=>{const entry=current();replaceCurrent(entry?.url||"vdm://home",{sessionId:tab().sessionId});};root.querySelector("[data-new]").onclick=()=>{const id="t"+Date.now().toString(36);tabs.push({id,title:"新しいタブ",history:[],cursor:-1,sessionId:"b"+Date.now().toString(36)});active=id;save();push("vdm://home");};
window.addEventListener("message",e=>{if(e.data?.type==="VDM_ROUTE")push(e.data.route,e.data);});window.addEventListener("vdm-open-payload",e=>e.detail?.route&&push(e.detail.route,e.detail));
